...
///                 
///   - `out_slot` : out-param, set to a slot index in `[0, num_slots)` only
///                  when returning CUDAQ_RX_READY.
///
...
/// host<->device or device<->wire copy, and ringing the transport's TX doorbell.
///
/// Interface:
...
///
/// The library's single-thread unified dispatch loop (`cudaq_host_unified_loop`,
/// used for CUDAQ_DISPATCH_PATH_HOST + CUDAQ_KERNEL_UNIFIED) owns one of these
/// and, on its own thread, repeatedly calls `rx_poll` to pull the next request
/// out of `ring`, dispatches it (HOST_CALL inline, GRAPH_LAUNCH via the graph
/// engine), and calls `tx_publish` to send each response back out `ring`.  A
/// provider that supports this shape returns a fully populated instance from
/// its `get_cpu_dataplane` callback; providers that do not leave that callback
/// NULL.
typedef struct {
