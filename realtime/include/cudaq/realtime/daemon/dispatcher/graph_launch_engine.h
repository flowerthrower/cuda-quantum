...
/// single-thread unified loop needs the markers (its publish_ready distinguishes
/// in-flight from done), so it must pass 0.
...
/// launch-error completions) before its worker is recycled, so a finished worker
/// is never freed before its response is sent.  A single non-blocking pass;
/// slot-addressed, so responses may be published out of the order their requests
/// were launched.  TX-publishing counterpart to cudaq_graph_launch_engine_sweep
/// (which only recycles); the single-thread unified loop uses this, while the
/// ring loop uses sweep because its transport threads own TX.
...
/// Drive `cpu_dataplane` from one thread: poll rx_poll for a ready slot, run its
/// dispatch mode inline (HOST_CALL run inline, GRAPH_LAUNCH via `engine`), then
/// publish it with tx_publish.  `engine` is created and destroyed by the caller
/// (NULL for a HOST_CALL-only table).  Blocks until `*shutdown_flag != 0`.
