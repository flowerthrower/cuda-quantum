...
// engine creates CUDA streams / graphs); a HOST_CALL-only table runs GPU-less on
// either HOST transport.
    ...
    if (unified) {
      ...
      dispatcher->host_thread =
          std::thread([rb, tbl, cfg, engine, shutdown_flag, stats] {
            cudaq_host_ring_dispatch_loop(&rb, &tbl, &cfg, engine, shutdown_flag,
                                          stats);
          });
    }
...
cudaq_status_t cudaq_host_release_worker(cudaq_dispatcher_t *dispatcher,
  ...
  if (dispatcher->config.dispatch_path != CUDAQ_DISPATCH_PATH_HOST ||
    ...
    return CUDAQ_ERR_INVALID_ARG;
  return cudaq_graph_launch_engine_release_worker(dispatcher->engine, worker_id);
}
